...
static uint32_t gLocalIp = MAKE_IPV4_ADDR(192, 168, 0, 133);
	...
	while (1) 
    {
		...
		for (i = 0;i < num_recvd;i ++) 
        {
			...
			if (ehdr->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_ARP)) 
            {
                ...
                {
					struct rte_mbuf *arpbuf = ng_send_arp(mbuf_pool, ahdr->arp_data.arp_sha.addr_bytes, ahdr->arp_data.arp_tip, ahdr->arp_data.arp_sip);
